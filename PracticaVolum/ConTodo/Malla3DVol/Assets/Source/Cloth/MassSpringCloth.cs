...
public class MassSpringCloth : MonoBehaviour
{
    ...
    private Dictionary<String, Node> dict2 = new Dictionary<String, Node>();
  //  public static Dictionary<String, Node> dict2 = new Dictionary<String, Node>();
...

    {
        ...
        for (int i = 0; i < vertices.Length; i++)
        {
            ...
            foreach (FixerCloth fixer in fixers)
            {
        ...
        for (int i = 0; i < triangs.Length; i += 3)//Se añaden 3 muellles por triangulo.
        {
            ...
            if (!dict2.ContainsKey(clave1))
            {

                dict2.Add(clave1, nodes[idc]);
                springs.Add(new Spring(nodes[ida], nodes[idb]));
            }
            else
            {
                Spring s = new Spring(nodes[triangs[c]], dict2[clave1]);
                s.Stiffness = flex;
                springs.Add(s);
            }
            if (!dict2.ContainsKey(clave2))
            {

                dict2.Add(clave2, nodes[ida]);
                springs.Add(new Spring(nodes[idb], nodes[idc]));
            }
            else
            {
                Spring s = new Spring(nodes[triangs[a]], dict2[clave2]);
                s.Stiffness = flex;
                springs.Add(s);
            }
            if (!dict2.ContainsKey(clave3))
            {

                dict2.Add(clave3, nodes[idb]);
                springs.Add(new Spring(nodes[idc], nodes[ida]));
            }
            else
            {
                Spring s = new Spring(nodes[triangs[b]], dict2[clave3]);
                s.Stiffness = flex;
                springs.Add(s);
            }
